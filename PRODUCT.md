...
- Data provider: Finnhub (free tier) for price/candle data and company news. Constraints to design around: ~60 API calls/minute rate limit, slightly delayed (not tick-level real-time) US equity quotes, and free-tier limits on historical depth — chart/indicator requests should be cached/throttled client- or server-side rather than re-fetched on every interaction.

## Capabilities and Constraints

- Candlestick price chart for NVDA with adjustable timeframes (1D/1W/1M/3M/1Y/5Y+), volume bars, smooth zoom/pan.
- Indicators: RSI (mandatory, own panel), MACD (own panel), SMA/EMA with configurable periods (default 20/50/200), Bollinger Bands, volume-weighted indicators where feasible. Overlays (MA, Bollinger) render on the price pane; oscillators (RSI, MACD) render in separate panes below. All indicators toggleable with adjustable parameters.
- Dedicated news page: NVDA-specific headlines from Finnhub's company-news endpoint, each with headline, source, timestamp, short summary, presented as a curated feed (not a raw list dump).
- Financial statements page: income statement, balance sheet, cash flow — at minimum revenue, net income, EPS, margins, debt, free cash flow — with trend visualization across recent quarters/years, not tables alone.
- Analysis tool: rule-based synthesis (no live LLM call) that interprets current indicator readings (RSI overbought/oversold, MACD crossover, trend direction) and contextualizes fundamentals (growth trends, P/E, P/S vs historical average) into a plain-language read.
