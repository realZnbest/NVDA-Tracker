...
- Alerts: user-creatable threshold rules (price cross, RSI overbought/oversold, MACD crossover, MA golden/death cross), manageable (create/edit/delete) from one Alerts panel/page, surfaced via in-app notification panel + badge, persisted in Turso so they survive a Render redeploy. The rest of the dashboard is public by design (shareable link), but alerts/notifications are the owner's private data, so `/alerts`, the notification bell, and their API routes require a single shared `ALERTS_PASSWORD` (signed cookie session) — visitors without it can't view alert rules, notification history, or even trigger evaluation.
