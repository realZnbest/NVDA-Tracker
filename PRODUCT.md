...
- Alert evaluation is server-driven, not visitor-driven: an external scheduler hits `/api/alerts/check` every 5–10 min independent of whether anyone has a tab open, protected by a shared `CRON_SECRET` header. Default/free mechanism: a GitHub Actions workflow in this repo (`.github/workflows/alert-check.yml`); a paid Render Cron Job (~$1/mo) is documented as a more punctual alternative — GitHub's scheduled runs are best-effort and can lag during platform load. Fired alerts are written to the in-app notification store and (when `OWNER_EMAIL`/`RESEND_API_KEY` are set) emailed via Resend — email is the primary reliable channel since the in-app badge only helps while a tab happens to be open.
