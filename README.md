...
The dashboard itself (price, chart, news, financials) is meant to be publicly viewable if you share the link. Alerts and notifications are not — they're your own trading rules and trigger history — so `/alerts`, the notification bell, and their API routes all require `ALERTS_PASSWORD` to view or manage. Entering the password once sets a signed, httpOnly cookie in your browser (30-day expiry); nothing is exposed to visitors who don't have it, including alert evaluation itself (it only runs for an authenticated session, so a stranger can't even trigger it to see what fires). Without `ALERTS_PASSWORD` set, that whole section just shows a "not configured" message.
...
4. **Environment variables:** add `FINNHUB_API_KEY`, `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`, and `ALERTS_PASSWORD` (Environment tab); optionally add any of the AI provider keys from `.env.local.example` for the AI summary cards — `GEMINI_API_KEY` alone is enough, the rest are just extra fallback quota.
